...
export default function Home({ onPlay }) {
    ...
    const trending = mockPodcasts.slice(0, 4);
    const recent = mockPodcasts.slice(2, 6);
    const popular = [...mockPodcasts].sort((a, b) => b.subscribers - a.subscribers).slice(0, 4);
      ...
      `}</style>
            <div className="page-container">
                ...
                <section className="home-section">
                    ...
                    <div className="podcast-scroll">{trending.map(p => <div key={p.id} className="scroll-item"><PodcastCard podcast={p} onPlay={onPlay} /></div>)}</div>
                </section>
                <section className="home-section">
                    <div className="section-header"><h2 className="section-title"><FiClock size={20} /> Recently Added</h2></div>
                    <div className="podcast-scroll">{recent.map(p => <div key={p.id} className="scroll-item"><PodcastCard podcast={p} onPlay={onPlay} /></div>)}</div>
                </section>
                <section className="home-section">
                    <div className="section-header"><h2 className="section-title">🔥 Most Popular</h2></div>
                    <div className="grid-2">{popular.map(p => <PodcastCard key={p.id} podcast={p} onPlay={onPlay} />)}</div>
                </section>
            </div>
        </>
    );
